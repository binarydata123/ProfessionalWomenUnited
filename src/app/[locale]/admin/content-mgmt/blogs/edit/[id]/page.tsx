...
import { EditBlogByAdmin, getSingleBlogDetailsByAdmin, deleteBlogsByAdmin } from '../../../../../../../../lib/adminapi';
...
export default function Page({ params }: { params: { id: string } }) {
	...
	const [bannerImageCheck, setBannerImageCheck] = useState(false);

	const [author, setAuthorImage] = useState('');

	const [authorImageCheck, setAuthorImageCheck] = useState(false);
	...
	useEffect(() => {
		if (user) {
			...
			fetchSingleBlogDetails(user?.id, params.id);
		}
		getAllServicesData();
		handleAuthors()
	}, []);
	...
	const fetchSingleBlogDetails = async (userId: any, LegalForumId: string) => {
		const res = await getSingleBlogDetailsByAdmin(userId, LegalForumId);
		if (res.status == true) {
			setFormData({
				...formData,
		...
		<div>
			...
			<form className="mt-2" onSubmit={handleSubmit}>
				<div className="text-right">
					<button
						type="button"
						className="btn-primary mt-1 mx-2 mb-2"
				...
				<TextEditor height={300} value={desc} onChange={handleDescriptionChange} />

				{errors.description && (
					<small className="error-message text-danger mt-5 pt-2 d-block">{errors.description}</small>
				)}
					...
					<div className="col-sm-5 pr-0 m-pr-   15">
						<div className="form-fild-des">
							<input
								...
								value={formData.publish_date ? formData.publish_date.slice(0, 10) : ''}
				...
				<select
					className="form-fild  w-100 mt-2"
					value={formData.author_id}
					onChange={e => setFormData({ ...formData, author_id: e.target.value })}>
					<option value={''}>Select Author</option>
					{
						authors.map((author: any, index: number) => (
							<option value={author.id} selected={author.id == formData.author_id ? true : false} key={index}>{author.name}</option>
						))
					}
				</select>
				{errors.author_id && (
