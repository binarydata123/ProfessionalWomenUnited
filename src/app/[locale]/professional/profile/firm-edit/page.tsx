...
import { createOrUpdateFirmByLawyer } from '../../../../../../lib/lawyerapi';
import { toast } from 'react-toastify';
import AuthContext from '@/context/AuthContext';
import { getAllCountries, getAllServices } from '../../../../../../lib/frontendapi';
import { getAllSpecialization, getFirmToLawyer } from '../../../../../../lib/lawyerapi';
...
// export default function page({ firmDetails, onCancel, firmId }: any) {
export default function Page() {
	...
	const [firmDetails, setFirmDetails]: any = useState([]);
	...
	const fetchFirmDetails = async () => {
		try {
			...
			const response = await getFirmToLawyer(data);
			if (response.status === true) {
				setFirmDetails(response.data);
				setFirmId(response.data[0].id)
			}
		} catch (error) {
	...
	function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
		...
		if (isValid) {
			...
			// If the firm being edited is not created by the admin, retain the existing created_by value
			...
			createOrUpdateFirmByLawyer(data)
				.then(res => {
					if (res.status == true) {
		...
		if (firmDetails && firmDetails.length > 0 && firmId) {
			...
			if (matchedFirm) {
				...
				const specializationIds = matchedFirm.specialization_id
					...
					: [];
				const serviceIdsArray = matchedFirm.service_id ? matchedFirm.service_id.split(',').map(Number) : [];
			...
			<div className="col-lg-7">
				<div>
					<form className="mt-2 p-5" onSubmit={handleSubmit}>
						...
						<div className="row mb-1 align-items-center">
							<div className="col-sm-2 col-3 pr-0 mt-1">
								{imagePreview ? (
									...
									<Image
										src={process.env.NEXT_PUBLIC_IMAGE_URL + '/images/default/Profile Avatar.png'}
										height={100}
										width={100}
										alt="Preview"
										style={{ objectFit: 'contain' }}
						...
						{errors.image && <small className="error-message text-danger d-block">{errors.image}</small>}
                    ...
                    <small className="error-message text-danger d-block">{errors.website_url}</small>
						...
						<TextEditor height={100} value={formData.bio && formData.bio} onChange={handleDescriptionChange} />
						{errors.bio && <small className="error-message text-danger d-block mt-5">{errors.bio}</small>}
						<br></br>
						<div className="modal-ft">
							<div className="row mt-4 pt-3">
								<div className="col-sm-6"></div>
								<div className="col-sm-3 col-4 text-end">
									{/* <button
										type="button"
								...
								<div className="col-sm-3 col-8">
									<button
										...
										{!isLoading ? (firmId ? 'Update' : 'Save') : 'Please wait ...'}
									</button>
								</div>
							</div>
						</div>
