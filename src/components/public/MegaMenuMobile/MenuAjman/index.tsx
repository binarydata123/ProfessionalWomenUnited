...
export default function MegaMenuAjman({ onclose }: any) {
	...
	const handleLinkClick = () => {
		...
		const collapseOne = document.getElementById('collapseOne4');
		if (collapseOne) {
					...
					<div className="accordion-item">
						<h2 className={`accordion-header ${isActive ? 'setMegaMenuAdd' : ''}`}>
							<button
								...
								style={{ color: '#4F4F4F' }}>
								<span onClick={() => { window.location.href = '/lawyers-in-ajman'; }}>{t('Ajman')}</span>
							</button>
						</h2>
						<div
							id="collapseOne4"
