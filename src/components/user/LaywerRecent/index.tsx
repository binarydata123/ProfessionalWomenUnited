...
export default function LawyerSaved({ lawyerdata, handleDelete, handleSave }: Props) {
	...
	useEffect(() => {
		if (user) {
			...
			getCheckSavedLawyer(user?.id, lawyerdata.id);
		}
		getCheckReportData(lawyerdata.id);
	...
	const openReportPopup = (id: any = lawyerdata.id) => {
	...
	const getCheckReportData = async (lawyer_id: any = lawyerdata.id) => {
		try {
						...
						<ul className="rating-location">
										...
										<li>
											{checkreport ? (
												<p
													className="weight-semi-bold"
													style={{ cursor: 'pointer' }}
													onClick={() => handleUnreportMessages(lawyerdata.id)}
												>
													UnReport
												</p>
											) : (
												<p
													className="weight-semi-bold"
													style={{ cursor: 'pointer' }}
													onClick={() => openReportPopup(lawyerdata.id)}
												>
													Report
												</p>
											)}
							...
							<li className="mr-1">
								<i
									...
									onClick={() => handleIconClick(lawyerdata.id)}
									ref={anchorRef}
								></i>
							</li>
							<li className="">
