...
export const getAllStates = async () => {
	const res = await axios.get(process.env.NEXT_PUBLIC_API_URL + '/states', {
		headers: {Accept: 'application/json'}
	});
	return res.data; // ✅ return data directly
};

export const getProfessionsByCity = async cityId => {
	try {
		const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/city/${cityId}`, {
			headers: {Accept: 'application/json'}
		});
		return res.data;
	} catch (error) {
		console.error('Error fetching professions by city:', error);
		throw error;
	}
};
...
export const getAllProfessions = async () => {
	return new Promise((resolve, reject) => {
		const req = axios.request(`${process.env.NEXT_PUBLIC_API_URL}/professions`, {
			method: 'get',
			headers: {Accept: 'application/json'}
		});
		req.then(res => resolve(res.data)).catch(err => reject(err));
...
// export const getMembersByService = async service_id => {
// 	try {
// 		const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/members-by-service`, {
// 			headers: {
// 				Accept: 'application/json',
// 				Authorization: `Bearer ${getToken()}`
// 			},
...
// 	}
// };
export const getMembersByService = async (service_id, city) => {
	try {
		const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/members-by-service`, {
			headers: {
				Accept: 'application/json',
