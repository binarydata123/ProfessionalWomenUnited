		...
		<div className="lawyer-info-wrapper">
			...
			{lawyer_info.linkedin_url && (
				<p className="weight-medium font-medium green-medium-2 mt-3">
					<a href={lawyer_info.linkedin_url} target="_blank" className="green-medium-2" rel="noreferrer">
						<Image src="/images/Blogs/iconoir_linkedin.svg" alt="iconoir_linkedin" width={20} height={20} />{' '}
						Linkedin
					</a>
				</p>
			)}

			{lawyer_info.license_number && (
				<p className="weight-medium font-medium green-medium-2 mt-3">
					License Number:{' '}
					<span className="weight-light font-small color-light">{lawyer_info.license_number}</span>
				</p>
			)}
			...
			{lawyer_info.specializ_name && (
				<>
					...
					<p className="weight-light font-small color-light">{lawyer_info.specializ_name}</p>
				</>
			)}
			...
			{lawyer_info?.bio && (
				<>
					...
					<p className="weight-light font-small color-light">
						{/* {showMore
							? lawyer_info?.bio
							: `${lawyer_info?.bio.slice(0, 150)}${lawyer_info?.bio.length > 150 ? '...' : ''}`} */}
						{showMore ? (
							<div dangerouslySetInnerHTML={{ __html: lawyer_info.bio }} />
						) : (
							<div dangerouslySetInnerHTML={{ __html: `${lawyer_info.bio.slice(0, 150)}${lawyer_info.bio.length > 150 ? '...' : ''}` }} />
						)}
					</p>

					{lawyer_info?.bio.length > 150 && (
						<div>
							<a
								href="JavaScript:void(0)"
								onClick={toggleShowMore}
								className="green-medium-2 font-x-small"
								style={{ padding: '0px' }}
							>
								{' '}
						...
						<div className="row">
							...
							{lawyer_info?.status && (
								<div className="col-lg-3 col-md-6 col-6 mb-2 mt-2">
									...
									<p className="font-medium  weight-medium text-sonic-silver mt-2 text-capitalize">
										{lawyer_info?.status}
									</p>
								</div>
							)}
