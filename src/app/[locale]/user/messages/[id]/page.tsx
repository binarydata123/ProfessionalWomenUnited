...
import { RiFlag2Line, RiFlag2Fill, RiVideoAddLine } from 'react-icons/ri';
...
import {
	GetAllUserInquiriesMessages,
	reportInquiriesByuser,
	unReportInquiriesByUser,
	SaveInquiriesByUser,
	sendUserInquiryResponse,
	markReadUserInquiry
} from '../../../../../../lib/enduserapi';
import {
...
interface Message {
	...
	message: string;
	created_at: any;
	inquiry_to_image_name: string;
	inquiry_to_gender: string;
	inquiry_by: any;
	type: any;
}
...
export default function Page({ params }: { params: { id: number } }) {
	...
	const fetchMessages = async (user_id: any, inqId: number) => {
		try {
			const response = await GetAllUserInquiriesMessages(user_id, inqId);
			setMessages(response.data);
			setInquiryByUserId(response.data[0].to_id);
	...
	const handleReadInquiry = async (user_id: any, inqId: number) => {
		try {
			const response = await markReadUserInquiry(user_id, inqId);
		} catch (error) {
			console.error('Error fetching messages:', error);
		}
	};
	...
	const handleSaveMessages = async () => {
		...
		try {
			const response = await SaveInquiriesByUser(user_id, inqId, saveMessage);
			if (response) {
			}
		} catch (error) {
	...
	const sendMessage = async () => {
		...
		try {
			...
			const response = await sendUserInquiryResponse(formData);
			if (response) {
				setMessageText('');
				setAttachment(null);
	...
	return (
		<>
			<div className="single-inquiry-wrapper pt-5 mt-4">
				<div className="right-body">
					<Link href={'/user/messages'} className="one-step-back-button">
						<ChevronLeftIcon width={20} /> Inbox
					</Link>
					<div className="client-info d-flex justify-content-between">
						<div className="left-info d-flex align-items-center gap-2">
							{messages.length > 0 ? (
								<Image
					...
					<div className="message-send-area">
						{attachment && (
							<div className="attachment-box">
								...
								<button className="btn btn-sm btn-danger mb-4" onClick={handleRemoveAttachment}>
									Remove
								</button>
							</div>
						)}
						<div className="input-group mb-lg-3">
							<input
								type="text"
