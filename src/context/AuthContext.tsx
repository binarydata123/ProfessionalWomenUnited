	...
	useEffect(() => {
		...
		if (token) {
			...
			axios
				...
				.then(response => {
					...
					if (data.status == true) {
						...
						if (data.user.role === 'enduser' && data.user.otp && data.user.two_factor_auth === 'yes') {
							router.push('/auth/two-factor-authentication');
						}
						// If two-factor is disabled, redirect to dashboard directly
						else if (data.user.role === 'enduser' && data.user.two_factor_auth === 'no') {
							router.push('/dashboard'); // Or your dashboard route
						} else if (data.user.status != 'deactive' || pathname.includes('/auth/')) {
							setUser(data.user);
						}
					} else {
			...
			if (pathname.includes('/admin/') || pathname.includes('/professional/') || pathname.includes('/user/')) {
				toast.error(
					'You are not logged in or your session has expired. Please log in again to access this page.'
				);
			...
			if (user?.role === 'professional') {
				if (payment_status === 'pending') {
					...
					if (user?.two_factor_auth === 'yes') {
						toast.success(
							'User logged in successfully. Two-factor authentication code has been sent to your email.'
						);
					} else {
						axios.defaults.headers.common.Authorization = `Bearer ${token}`;
						Cookies.set('session_token', token);
