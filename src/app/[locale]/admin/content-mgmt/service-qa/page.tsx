...
import { getAlServiceQaFilteredForAdmin, CreateorUpdateServiceQaByAdmin, deleteServiceQaByAdmin } from '../../../../../../lib/adminapi';
...
export default function lawyers() {
	...
	const handleChange = (inputName: any, value: any, user_id: any) => {
		...
		getAlServiceQaFilteredForAdminData(data);
	};

	const getAlServiceQaFilteredForAdminData = async (data: any) => {
		try {
			const response = await getAlServiceQaFilteredForAdmin(data);
	...
	const Edittags = (id: any, service_question: any, service_answer: any, service_id: any) => {
		setQaId(id);
	...
	const changeUserStatus = (id: any) => {
		...
		Swal.fire({
			title: 'Are you sure?',
			text: 'You Want to remove the QA',
			icon: 'warning',
	...
	const handleServiceAnswerChange = (newValue: any) => {
		setFormData({ ...formData, service_answer: newValue });
	};


	function validateForm() {
		const newErrors: { [key: string]: string } = {};
		if (!formData.service_question) {
			newErrors.service_question = 'Question is required';
		}
		if (!formData.service_answer) {
			newErrors.service_answer = 'Answer is required';
		}
	...
	function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
		...
		if (isValid) {
			const data = {
				service_question: formData.service_question,
				service_answer: formData.service_answer,
				qa_id: qa_id,
				service_id: formData.service_id,
				user_id: user_id
			};
			CreateorUpdateServiceQaByAdmin(data)
				.then(res => {
					if (res.status == true) {
	...
	return (
		<div>
					...
					<div className="col-sm-6 col-md-6 col-lg-3">
						...
						>
							<p className="d-flex align-items-center font-bold">
								<span className="plues-icon">+</span> New QA
							</p>
						</DefaultButton>
					</div>
				</div>
			...
			<p className="font-small weight-light social-link mt-3 mb-3">
				Displaying <span className="span-color-dash weight-bold">{filter_lawyer.length}</span> Service QA
			</p>

			<div className="table-part mt-3">
				<Table columns={['Question', 'Category', 'Created On', 'Actions']} data={currentLawyer}>
					{(rowData, index) => (
						<tr key={index}>
							<td data-th="Question">
								<OverlayTrigger
									...
									>
										{rowData.question && rowData.question.length > 100
											? rowData.question.substring(0, 100) + '...'
											: rowData.question}
									</span>
								</OverlayTrigger>
							</td>
							...
							<td data-th="Actions " className="text-right ">
								<EditButton
									Tooltip="Edit"
									onClick={e => Edittags(rowData?.id, rowData?.question, rowData?.answer, rowData?.service_id)}
								/>
								<DeleteButton
									Tooltip="Delete Tag"
			...
			<Popup
				...
				title={qa_id ? 'Edit Service QA' : 'Create Service QA'}
				...
				<form className="mt-2" onSubmit={handleSubmit}>
					<div className="row">
						...
						<div className="col-sm-4 text-right">
							<p className="Chinese-silver font-x-small weight-light">200</p>
						</div>
					</div>
					<input
						type="text"
						...
						className="form-fild  w-100"
						value={formData.service_question}
						maxLength={500}
						onChange={e => setFormData({ ...formData, service_question: e.target.value })}
					/>

					{errors.service_question && <small className="error-message text-danger">{errors.service_question}</small>}
					...
					<TextEditor
						height={150}
						value={formData.service_answer && formData.service_answer}
						onChange={handleServiceAnswerChange}
					/>

					<br /> <br />

					{errors.service_answer && <small className="error-message text-danger mt-2">{errors.service_answer}</small>}
