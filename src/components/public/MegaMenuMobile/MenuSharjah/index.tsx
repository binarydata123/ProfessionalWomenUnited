...
export default function MegaMenusharjah({ onclose }: any) {
	...
	const handleLinkClick = () => {
		...
		const collapseOne = document.getElementById('collapseOne3');
		if (collapseOne) {
					...
					<div className="accordion-item">
						<h2 className={`accordion-header ${isActive ? 'setMegaMenuAdd' : ''}`}>
							<button
								...
								style={{ color: '#4F4F4F' }}>
								<span onClick={() => { window.location.href = '/lawyers-in-sharjah'; }}>{t('Sharjah')}</span>
							</button>
						</h2>
						<div
							id="collapseOne3"
