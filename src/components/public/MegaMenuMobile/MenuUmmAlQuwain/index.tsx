...
export default function MegaMenuUmmAlQuwain({ onclose }: any) {
	...
	const handleLinkClick = () => {
		...
		const collapseOne = document.getElementById('collapseOne5');
		if (collapseOne) {
					...
					<div className="accordion-item">
						<h2 className={`accordion-header ${isActive ? 'setMegaMenuAdd' : ''}`}>
							<button
								...
								style={{ color: '#4F4F4F' }}>
								<span onClick={() => { window.location.href = '/lawyers-in-umm-al-quwain'; }}>{t('Umm_Al_Quwain')}</span>
							</button>
						</h2>
						<div
							id="collapseOne5"
